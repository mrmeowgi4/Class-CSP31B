...
public:
    ...
    class Iterator {
    public:
        Iterator(vector<Entry<K, V>>* table, int capacity, int index, typename vector<Entry<K, V>>::iterator entryIterator)
            : table(table), capacity(capacity), index(index), entryIterator(entryIterator) {}

        bool operator!=(const Iterator& other) const {
            return entryIterator != other.entryIterator || index != other.index;
        }
        ...
        Iterator& operator++() {
            ++entryIterator;
            while (entryIterator == table[index].end() && index < capacity - 1) {
                ++index;
                entryIterator = table[index].begin();
            }
            return *this;
        }
    ...
    private:
        ...
        int capacity;
        int index;
        typename vector<Entry<K, V>>::iterator entryIterator;
    };
    ...
    Iterator begin() {
        ...
        return Iterator(table, capacity, index, table[index].begin());
    }

    Iterator end() {
        return Iterator(table, capacity, capacity, table[capacity - 1].end());
    }
